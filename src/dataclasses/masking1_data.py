...
#         for tok_ids, token in enumerate(tokenized_sentence):
#             if token in pic_map["word"].values:
#                 pid = int(pic_map.loc[pic_map["word"] == token, "pictogram_id"].iloc[0])
#                 image_path = self.image_path + pid + ".png"
#                 if os.path.exists(image_path):
#                     tokens_with_image.append(token)
#                     tokens_with_image_ids.appen(tok_ids)
        

#         # Mask last token with associated image
...
#                 image_path = self.picto_path + str(pid) + ".png"
